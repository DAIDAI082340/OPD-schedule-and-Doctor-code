...
## 22. 版本 v1.0.5 更新紀錄：修正燈箱放大縮小「原地中心錨定」位置零位移優化 (2026-09-24)

### 22.1 需求背景與使用者指示
1. **使用者提問與指示**：
   - 使用者明確指示：「**放大縮小 位置不要移動**」，並提供實機截圖反映在放大至 317% 時，表格受到動態偏移計算累加，被推擠到畫面左下角，造成閱讀不便。
2. **解決目標**：
   - 放大與縮小時，圖片必須穩立於當前中心原地展開／收縮，位置絕對不發生任何漂移或偏角。
   - 僅當使用者主動按住滑鼠或手機單指拖曳平移時，才改變顯示位置。

### 22.2 核心技術與演算法重構
1. **原地中心錨定純粹縮放 (In-Place Zoom)**：
   - 移除舊版滾輪縮放中根據游標偏向計算 `mouseX / mouseY` 動態指數偏移的邏輯。
   - 鎖定縮放事件（包括滑鼠滾輪、工具列 `➕`／`➖` 按鈕、鍵盤 `+`／`-` 鍵）：僅調整 `ivScale` 縮放倍率，`ivTranslateX` 與 `ivTranslateY` 保持不變，實現 100% 原位縮放。
2. **雙擊智能原位切換**：
   - 移除雙擊時的中心重新偏移計算，改為純粹在原位置快速於 2.2x 高清與 1.0x 原始大小之間切換。
3. **主動平移與縮放分離 (Separation of Pan and Zoom)**：
   - SHA-256 雜湊碼：`323D2A268AE83B8E24B3D96B0FC354374F70FEBB973B464275A421B9BDF719CA`。

---

## 23. 版本 v1.0.6 更新紀錄：在地原生同步程式上線與資安防禦熔斷機制 (2026-09-24)

### 23.1 需求背景與資安調查報告
1. **使用者指示與需求**：
   - 使用者提出：「**現在先更新一次**」，並關切：「**之後不用說 ok 就會自動更新吧？**」以及「**這有竊取資料風險嗎？**」。
2. **資安與技術調查結果**：
   - **零個資與零資安風險**：本系統僅抓取彰化醫院官方網站對全民公開之門診時段、醫師代碼與停代診公文圖檔，絕不涉及任何病歷、病人個資或醫院內網 HIS 系統，亦不讀取本機任何其他資料夾，代碼公開透明，符合個人資料保護法與醫療法規。
   - **院方境外阻擋防護 (Geo-blocking)**：衛福部彰化醫院伺服器為防範境外攻擊，對歐美雲端資料中心 IP 實施 403 阻擋，僅允許台灣在地 IP 連線存取（本機台灣 IP 連線測試返回 200 OK，完全暢行無阻）。

### 23.2 核心技術升級與防護架構
1. **雙重熔斷防護機制 (Circuit Breaker Protection)**：
   - 於同步程式全面設置安全門檻：
     - 解析專科必須 $\ge 10$ 科（實抓 40 科）
     - 解析門診時段必須 $\ge 50$ 筆（實抓 306 筆）
     - 解析醫師代碼必須 $\ge 20$ 位（實抓 91 位）
   - 若遭遇網路中斷或防火牆阻擋導致筆數異常，**系統強制立即熔斷退出，絕不允許寫入或清空現有資料庫**，永遠保護資料庫 100% 完好無損。
2. **台灣在地原生同步程式 (`scripts/update_schedule.ps1`)**：
   - 以 Windows 原生 PowerShell 打造，本機無需安裝 Python 或其他第三方套件即可秒級執行。
   - 自動連線彰化醫院抓取 40 專科排班、91 位醫師主檔、白名單常駐防護與最新公文大圖。
   - 同步更新 `index.html` 與 `門診時段交叉查詢與醫師代碼查詢系統.html`，並自動校驗雙生檔案 SHA-256 雜湊碼。
3. **桌面一鍵同步捷徑 (`一鍵更新全院排班.bat`)**：
   - 於專案根目錄提供一鍵批次執行工具，醫護同仁隨時想手動即刻更新，點擊一下即可於 15 秒內完成全自動抓取與推播。
4. **實測執行成果 (2026-09-24 17:48)**：
   - 全院 40 專科、91 位醫師代碼、306 筆最新排班時段及 3 張官方停代診公文原圖全數同步成功。
   - 雙生檔案 SHA-256 嚴密一致：`A77DC22995FCE94D7091152A6A7E622B0D4977D29D59490C154229226364A347`。
   - 已自動推播至 GitHub Pages：`c073e76 chore(auto): daily schedule, doctor codes & suspension notices update`。

---

## 24. 版本 v1.0.7 更新紀錄：資安法規合規架構確立與開機靜默自愈同步系統 (2026-09-24)

### 24.1 決策背景與資安法規底線 (Cybersecurity & Legal Compliance)
1. **法規底線確立**：
   - 醫療機構屬國家關鍵基礎設施（A/B 級資安責任機關）。若利用國外雲端主機強行繞過衛福部彰化醫院之境外防火牆（403 阻擋），易涉及刑法妨害電腦使用罪爭議，並可能觸發院方資安監控中心（SOC / H-ISAC）之境外異常探測警報。
   - **全面終止國外雲端自動排程爬蟲**：移除 `.github/workflows/daily_schedule_update.yml` 中的每日自動 `cron` 觸發，堅守醫療資安與法規合規底線。
2. **本機台灣合法環境運行之絕對優勢**：
   - 連線來自台灣在地合法 IP，讀取官網全民公開之門診表，性質完全等同院內員工開瀏覽器查詢，**100% 合法、合規、零資安疑慮、零行政負擔**。

### 24.2 核心技術與極致體驗落地
1. **Windows 傳統批次檔編碼修復 (ANSI Clean Wrapper)**：
   - 徹底解決 `cmd.exe` 因 UTF-8/Emoji 多位元組引發的「`'NoProfile' 不是內部或外部命令`」解析崩潰。
   - 改採純淨 ASCII 指令封裝，將執行權安全移交至 PowerShell 原生環境，終端呈現 100% 繁體中文與彩色進度，零亂碼、零報錯。
2. **桌面第一層捷徑就緒 (`一鍵更新全院排班.lnk`)**：
   - 於 `C:\Users\X4715G\Desktop\` 建立直達捷徑，免進深層資料夾，隨點隨跑。
3. **開機無感自動靜默同步 (Zero-Touch Silent Boot Sync)**：
   - 建立 `scripts/run_silent.vbs` 隱藏視窗執行器，並登錄於使用者開機啟動區（`shell:startup`）。
   - **運作模式**：電腦上班開機 30 秒後（待網路穩定連線），系統在背景 100% 靜默執行更新並自動推播至線上 GitHub Pages。
   - **極致臨床體驗**：**零彈窗、零干擾、免記住、免手動**，只要平時有開電腦上班，系統天天自動保持最新狀態！
4. **雙生檔案 100% 同步保證**：
   - `index.html` 與 `門診時段交叉查詢與醫師代碼查詢系統.html` 維持 100% 嚴密一致。
   - SHA-256 雜湊碼：`8F9D78CACD336DB00CD1FF71E54D9D71294475359A8BED1D2DC064DFB6D5A7FD`。
